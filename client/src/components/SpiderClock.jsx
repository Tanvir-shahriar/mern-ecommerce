...
export const SpiderClock = () => {
  ...
  useEffect(() => {
    ...
    const handMinEl = select("#hand-min");
    const handHrEl = select("#hand-hr");

    const face01 = select("#face01")?.getAttribute("d");
    const handSec01 = select("#handSec01")?.getAttribute("d");
    const handMin01 = select("#handMin01")?.getAttribute("d");
    const handHr01 = select("#handHr01")?.getAttribute("d");

    if (faceEl && face01) gsap.set(faceEl, { attr: { d: face01 } });
    if (handSecEl && handSec01) gsap.set(handSecEl, { attr: { d: handSec01 } });
    if (handMinEl && handMin01) gsap.set(handMinEl, { attr: { d: handMin01 } });
    if (handHrEl && handHr01) gsap.set(handHrEl, { attr: { d: handHr01 } });
    ...
    // Show elements once initialized
    gsap.set([select(".gsapWrapper"), select(".vline")], { autoAlpha: 1 });

    const t24Anim = gsap.to(select(".cw.t24"), {
      duration: 1,
      rotation: "-=15",
    ...
    const t20Anim = gsap.to(select(".cw.t20"), {
      duration: 1,
      rotation: "-=18",
      transformOrigin: "50% 50%",
      ease: "bounce",
    ...
    const t12Anim = gsap.to(select(".ccw.t12"), {
      duration: 1,
      rotation: "+=30",
      transformOrigin: "50% 50%",
      ease: "bounce",
    ...
    // Leg/Hand morphing timelines
    const tg1 = gsap.timeline({ repeat: -1, repeatDelay: 5, defaults: { duration: 1.5, ease: "bounce" } })
      .delay(1)
      .call(() => {
        ...
        if ((rotation > 30 && rotation < 150) || (rotation > 210 && rotation < 330)) {
          gsap.timeline({ repeat: 0, defaults: { duration: 0.25, ease: "bounce.in" } })
            .to(handSecEl, { morphSVG: select("#handSec02") })
            .to(handSecEl, { morphSVG: select("#handSec01") });
        }
      })
      .set(sec, {
        onComplete() {
    ...
    const tg2 = gsap.timeline({ repeat: -1, repeatDelay: 5, defaults: { duration: 1.5, ease: "bounce" } })
      .delay(5)
      .call(() => {
        let rotation = parseFloat(gsap.getProperty(min, "rotation").toFixed(1));
        if ((rotation > 5 && rotation < 175) || (rotation > 185 && rotation < 355)) {
          gsap.timeline({ repeat: 0, defaults: { duration: 0.25, ease: "bounce.in" } })
            .to(handMinEl, { morphSVG: select("#handMin02") })
            .to(handMinEl, { morphSVG: select("#handMin01") });
        }
      })
      .set(min, {
        onComplete() {
    ...
    const tg3 = gsap.timeline({ repeat: -1, repeatDelay: 5, defaults: { duration: 1.5, ease: "bounce" } })
      .delay(7)
      .call(() => {
        let rotation = parseFloat(gsap.getProperty(hr, "rotation").toFixed(1));
        if ((rotation > 2 && rotation < 178) || (rotation > 182 && rotation < 358)) {
          gsap.timeline({ repeat: 0, defaults: { duration: 0.25, ease: "bounce.in" } })
            .to(handHrEl, { morphSVG: select("#handHr02") })
            .to(handHrEl, { morphSVG: select("#handHr01") });
        }
      })
      .set(hr, {
        onComplete() {
    ...
    return () => {
      // Kill all running anim instances on unmount
      t24Anim.kill();
      t20Anim.kill();
      t12Anim.kill();
      minAnim.kill();
  ...
  return (
    <section className="spider-clock-section" ref={containerRef}>
      ...
      <div className="gsapWrapper">
        <div id="wBody">
          <div id="watch">
            <svg id="watchSVG" overflow="visible" version="1.1" viewBox="0 0 300 300" xmlnsXlink="http://www.w3.org/1999/xlink">
      ...
      <svg width={0} height={0} style={{ position: 'absolute', width: 0, height: 0, overflow: 'hidden' }} role="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <filter id="shadow" x="-5%" y="-5%" width="110%" height="110%">
            <feDropShadow dx="0" dy="0" floodColor="#a0a0a0" stdDeviation="1" />
          </filter>
          <filter id="shadow2" x="-25%" y="-25%" width="150%" height="150%">
            <feDropShadow dx="0" dy="0" floodColor="#a0a0a0" stdDeviation="1.25" />
          </filter>
          <filter id="blur1">
